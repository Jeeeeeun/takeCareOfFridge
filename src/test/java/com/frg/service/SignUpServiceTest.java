...
public class SignUpServiceTest {
	...
	public void testIsExistEmail() {
		//중복 있는 아이디 (임의로 DB에 있는걸 지정해줘서 사용)
		String email = "test2@com";
		//중복 없는 아이디
		String email2 = "test1@com";
		//서비스레이어에 있는 메소드가 제대로 작동하는지 확인하고 원하는 값이 반환하는지 검토를 위해 테스트
		//중복이랑 true || 중복이 아니라면 false를 반환
		//이유는 중복된 것이 있다면 쿼리문에 따라 1이 나옴
		//아니라면 0이 나와서 그 값에 따라서 참 거짓으로 나눠짐
		boolean serviceResult = service.isExistEmail(email);
		boolean serviceResult2 = service.isExistEmail(email2);
		log.info("이메일 - " + serviceResult);
		log.info("이메일2 - " + serviceResult2);
		//String 메세지를 쓸수 있는 assertTrue를 사용해서 여러 테스트 결과중 어떤게 잘 못 됐는지 확인하기 위해서 사용
		//중복
		assertTrue("실패", serviceResult);
		//중복 아님
...
//		user.setUser_email("test3@com");
		...
		user.setUser_pw("test123!");
		user.setUser_email("test2@com");
		
		boolean registerd = service.registerUser(user);
		log.info("service.isExistId(user.getUser_아이디())" + service.isExistId(user.getUser_id()));
		log.info("service.isExistEmail(user.getUser_이메일())" + service.isExistEmail(user.getUser_email()));
		log.info("servie.policyPwd(user.getUser_비밀번호())" + service.policyPwd(user.getUser_pw()));
		log.info("service.회원가입(user)" + service.registerUser(user));
