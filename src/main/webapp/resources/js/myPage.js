var currentIndex = 0;
var frgName, frgShape, hRadio, vRadio, sRadio, frgAstate, frgBstate, aFrozenBtn, aCoolBtn, bFrozenBtn, bCoolBtn, frgInfoChangeBtn, frgCorrectionEndBtn, dangerousStandard, warningStandard, dangerousSpan, warningSpan, trfCorrectionEndBtn, announcement;
	...
	getUserId()
    	.then(function (userId) {
			...
			var data = {
      		...
      		// 다시 렌더링을 위한 값 지정
      		dangerousStandard.value = data.dangerous;
      		warningStandard.value = data.warning;
      		
      		// 값 크기 비교 (유효하게 들어갈 수 있는지 검사)
            if (data.dangerous <= data.warning) {
                alert("위험 판단 기준이 경고 판단 기준보다 숫자가 커야 합니다. 값을 다시 입력해 주세요.");
                return;
			...
			$.ajax ({
				...
				success: function(response) {
					...
					setTimeout(function() { // 시간차 두고 알림창 띄우기
