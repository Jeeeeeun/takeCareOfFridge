		...
		<header class="masthead">
			...
			<div id="contents">
				<div class="currentStateBox">
					<div class="d-flex flex-column">
						<label>냉장고 선택:</label>
						<select name="frgList" id="frgSelect" onchange="changeFrg(this.value,'${pageContext.servletContext.contextPath}/frg/innerCtrl');">
							<option value="all">전체</option>
							<c:forEach var="name" items="${frgNames}">
								<option value="${name}">
									<c:if test="${ frgName eq name }">
										selected
									</c:if>>
									${name}
								</option>
					...
					<div class="stateBtns" id="stateIn">
						<button id="all" name="show_in_state" onclick="filterDataByState('all');" class="stateBtn">
							전체
						</button>
						<button id="cool" name="show_in_state" onclick="filterDataByState('cool');" class="stateBtn">
							냉장
						</button>
						<button id="frozen" name="show_in_state" onclick="filterDataByState('frozen');" class="stateBtn">
							냉동
						</button>
					</div>
					<div>
				...
				<div class="wholeFoodListBox">
					<div class="tableContainer" style="max-height: 400px; overflow-y: scroll; margin-left: 20px;">
						<table id="foodTable" class="foodTable">
							<thead>
								<tr>
									<th style="width: 100px; text-align: center; border: 1px solid #ccc; position: sticky; top: 0; background-color: #f9f9f9;">
										<span class="black_circle"></span>
									</th>
									<th style="width: 300px; text-align: center; border: 1px solid #ccc; position: sticky; top: 0; background-color: #f9f9f9;">
										식품명
									</th>
									<th style="width: 550px; text-align: center; border: 1px solid #ccc; position: sticky; top: 0; background-color: #f9f9f9;">
										유통기한
									</th>
									<th style="width: 250px; text-align: center; border: 1px solid #ccc; position: sticky; top: 0; background-color: #f9f9f9;">
										D-day
									</th>
									<th style="width: 250px; text-align: center; border: 1px solid #ccc; position: sticky; top: 0; background-color: #f9f9f9;">
										보관상태
							...
							<tbody>
								<c:forEach var="item" items="${dataList}">
									<tr onclick="handleRowClick('${item.in_name}', '${item.in_expireDate}', '${item.d_DAY}', '${item.in_state}');">
										<td>
											<span class="circleColor" ddayData="${item.d_DAY}"></span>
										</td>
			...
			<div>
				<div class="detailInfoBox">
					...
					<form id="detailForm" action="<%=request.getContextPath()%>/innerFoodCtrl" method="post" onsubmit="return false;">
						<div class="detailInfoItemBox">
							<label for="detailInfoItemBox_frg_name">보관 냉장고</label>
							<input name="frgName" id="detailInfoItemBox_frg_name" type="text" class="detailInputBox" disabled>
						</div>
						<div class="detailInfoItemBox">
							<label for="detailInfoItemBox_in_name">식품명</label>
							<input id="detailInfoItemBox_in_name" type="text" class="detailInputBox" disabled>
						</div>
						<div class="detailInfoItemBox" style="display: flex; justify-content: center;">
							<label>보관상태</label>
							<input name="in_state" id="coolRadio" type="radio" class="detailInputBox" value="cool" style="width: 10%" disabled>냉장
							<input name="in_state" id="frozenRadio" type="radio" class="detailInputBox" value="frozen" style="width: 10%" disabled>냉동
						</div>
						<div class="detailInfoItemBox">
							<label for="detailInfoItemBox_in_company">제조사명</label>
							<input id="detailInfoItemBox_in_company" type="text" class="detailInputBox" disabled>
						</div>
						<div class="detailInfoItemBox">
							<label for="detailInfoItemBox_in_expireDate">
								유통/<br>소비기한
							</label>
							<div>
								<input id="detailInfoItemBox_in_expireDate" type="date" class="detailInputBox" style="height: 30%;" disabled>
							</div>
						</div>
						<div class="detailInfoItemBox">
							<label for="detailInfoItemBox_d_DAY">D-Day</label>
							<input id="detailInfoItemBox_d_DAY" type="number" class="detailInputBox" disabled>
						</div>
						<div class="detailInfoItemBox">
							<label for="detailInfoItemBox_in_count">수량</label>
							<input id="detailInfoItemBox_in_count" type="text" class="detailInputBox" disabled>
						</div>
						<div class="detailInfoItemBox">
							<label for="detailInfoItemBox_in_type">식품 유형</label>
							<input id="detailInfoItemBox_in_type" type="text" class="detailInputBox" disabled>
						</div>
						<div class="detailInfoItemBox">
							<label for="indexValue"></label>
							<input id="indexValue" type="hidden" name="in_index" value="" />
						</div>
						<div class="detailInfoBtnBox" style="position: relative; top: 550%;">
							<div style="position: relative; display: flex; justify-content: center; align-items: center;">
								<button class="ctrlBtn" id="updateBtn" onclick="updateBtnClicked();">
									수정
								</button>
								<button class="ctrlBtn" id="deleteBtn" onclick="deleteBtnClick();">
									삭제
								</button>
							</div>
							<div style="position: relative; display: flex; justify-content: center; align-items: center;">
								<button class="ctrlBtn" id="updateEndBtn" onclick="updateEndBtnClicked()" style="display: none;">
									수정 완료
								</button>
								<button class="ctrlBtn" id="cancelBtn" onclick="cancelBtnClick()" style="display: none;">
									취소
								</button>
							</div>
						</div>
