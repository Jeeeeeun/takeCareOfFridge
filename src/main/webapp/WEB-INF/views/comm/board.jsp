				...
				<table class="bg-white-30 rounded-3 mt-2 w-100 h-95">
					...
					<tbody>
						<c:if test="${not empty allPosts}">
							<c:forEach var="board" items="${ allPosts }" varStatus="status">
								<tr>
									...
									<td class="w-45 p-3 py-2">
										${ board.board_title }
									</td>
									<td class="w-7 p-3 py-2 text-center">
										${ board.board_hasAttach >= 1 ?
											'<i class="fa-solid fa-paperclip"></i>'
											: 
											'' 
										}
									</td>
									<td class="w-10 p-3 py-2 text-center">
										${ board.user_id }
									</td>
									<td class="w-10 p-3 py-2 text-center">
						...
						<c:if test="${empty allPosts}">
							<tr>
								<td colspan="8" class="p-3 py-2 text-center">
									등록된 게시글이 존재하지 않습니다.
								</td>
							</tr>
						</c:if>
